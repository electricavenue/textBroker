...
if __name__ == "__main__":
    ...
    introduction="The document must be delivered with 2 section, Product highlights with bullets and Overview description. Please follow this link, rewrite the Highlights with similar number of sentences in bullets and rewrite the overview section, the total number of words for the overview long description should be about 50% of the current"

    filename="lenses_links.csv"
    infile = csv.reader(open(filename))
    outfile = csv.writer(open(str(date.today())+"orderElectricAvenueb.csv", "wb"))
    main = Main()
    main.run()
